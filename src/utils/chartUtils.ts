export const generateFrequencyBins = (
  ...
  const min = customMin !== undefined && customMin !== "" ? Number(customMin) : dataMin;
  const max = customMax !== undefined && customMax !== "" ? Number(customMax) : dataMax;
...
export const filterHostels = (data: any[], districtFilter: string, hostelFilter: string) => {
  let filtered = data;
