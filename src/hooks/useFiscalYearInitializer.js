...
const useFiscalYearInitializer = () => {
  ...
  const { value, options, dateRange } = selector(state => state?.app?.fiscalYear)
  const isInitializedRef = useRef(false)

  useEffect(() => {
    // Only initialize once and only if data is not already available
    if (isInitializedRef.current || (options && options.length > 0 && value && dateRange?.from && dateRange?.to)) {
      return
    }
